...
%^^A%   The default stretch is equal to~1. The version 3.1 of \package{caption} package offers
%^^A%   special font settings (see \package{caption} documentation) for captions. You may try the
%^^A%   same for the float font:
%   При совместном использовании данного пакета с~пакетом \package{setspace}
%   возникала ошибка \cdash--- последний переопределял размер шрифта, заданный в~настройках
%   обратно в~|\normalsize|. Версия 0.2d пакета \package{floatrow} пытается это исправить.
